...
class ChromaClient:
    ...
    def __init__(self,
        ...
        with suppress_stdout():
            ...
            # ChromaDB's persisted config only records the model name, so it
            # cannot tell 'openai:text-embedding-3-small' from a local model of
            # the same name, and it records nothing at all for backends that do
            # not expose `model_name`. Track the resolved provider:model in a
            # sidecar file as well and rebuild when that changes.
            self._reconcile_embedding_model_info()

    def _reconcile_embedding_model_info(self) -> None:
        """Rebuild the collection when the configured embedding model changed.

        Compares the provider:model recorded on the last run against the current
        configuration. On a mismatch the collection is dropped and recreated,
        because embeddings from different models are not comparable (and usually
        differ in dimension, which surfaces as an opaque ChromaDB error at query
        time). Always writes the current value back.
        """
        ...
        if stored_model_info and stored_model_info != current_model_info:
            ...
            sys.stderr.write("   → Auto-rebuilding collection with correct embedding model...\n\n")
            ...
            sys.stderr.write("  Run 'zotero-mcp update-db' to rebuild the database.\n\n")
    ...
    def _get_current_model_info(self) -> str:
        ...
        """
        # Defaults must stay in sync with _create_embedding_function, otherwise a
        # config that omits model_name looks like a change on the next run.
        if self.embedding_model == "openai":
            ...
            return f"embeddinggemma:{model_name}"
